...
export const determineBestOrganizationMethod = (images: EnhancedFile[]): string => {
    // Count images with different types of metadata
    const withExactDate = images.filter(img => !!img.metadata?.captureDate).length;
    ...
    const withAiTags = images.filter(img =>
