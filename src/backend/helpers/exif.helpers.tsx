 ...
 */
export const rotateImageBasedOnOrientation = (imgSrc: string, orientation: number): Promise<string> => {
    return new Promise((resolve, reject) => {
        ...
        img.onload = () => {
            ...
            }, 'image/jpeg', 0.95); // Use high quality for the rotated image
        };
            ...
            EXIF.getData(file as any, async () => {
                ...
                if (orientation !== 1) {
                    try {
                        ...
                        const rotatedImageUrl = await rotateImageBasedOnOrientation(tempUrl, orientation);
 ...
 */
export const extractImageMetadataFallback = (file: File): Promise<EnhancedFile> => {
    return new Promise((resolve) => {
        const enhancedFile = file as EnhancedFile;
...
export const identifyEvents = (files: EnhancedFile[]): EnhancedFile[] => {
    ...
    sortedFiles.forEach((file, index) => {
        ...
        if (
            ...
            file.metadata = {
                ...file.metadata,
                eventGroup: sortedFiles[index - 1].metadata?.eventGroup || `Event ${currentEventId}`
            };
        }
...
export const fetchLocationNames = async (
    ...
    const locationsToFetch: { latitude: number, longitude: number, key: string }[] = [];

    files.forEach(file => {
        if (file.metadata?.gpsLocation && !file.metadata.gpsLocation.locationName) {
            ...
            if (!locationMap.has(locationKey)) {
                locationsToFetch.push({ latitude, longitude, key: locationKey });
    ...
    const updatedFiles = files.map(file => {
        if (file.metadata?.gpsLocation && !file.metadata.gpsLocation.locationName) {
            const { latitude, longitude } = file.metadata.gpsLocation;
            const locationKey = `${latitude.toFixed(3)},${longitude.toFixed(3)}`;
            const locationName = locationMap.get(locationKey) || 'Unknown location';
