...
export async function GET(
  ...
  try {
    ...
    const processedImages = images.map((image) => {
      ...
      if (image.data as any) {
        try {
          // Create a small preview (limited size)
          const buffer = Buffer.from(image.data);
          const base64Preview = buffer.toString('base64').substring(0, 100000); // Limit size
          previewDataUrl = `data:${image.mimeType};base64,${base64Preview}`;
        } catch (error) {
          console.error(`Error creating preview for image ${image.id}:`, error);
        }
      }
