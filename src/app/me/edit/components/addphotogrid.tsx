...
const AddPhotoViewGrid: React.FC<addPhotoViewGridProps> = ({
   images,
   onClose,
}) => {
               ...
               <div className="min-w-max">
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 gap-4">
                     {images.map((file, index) => {
                        ...
                        const rotationTransform = file.metadata?.rotation
                           ? `rotate(${file.metadata.rotation}deg)`
                           : "";
                        const zoomTransform = file.metadata?.zoom
                           ? `scale(${file.metadata.zoom})`
                           : "";
                        const combinedTransform =
                           `${rotationTransform} ${zoomTransform}`.trim();
                        ...
                        return (
                           ...
                           >
                              ...
                              {file.metadata?.textAnnotation &&
                                 file.metadata.textAnnotation.position &&
                                 file.metadata.textAnnotation.textContent && (
                                    <div
                                       ...
                                       style={{
                                          ...
                                          color:
                                             file.metadata.textAnnotation.style
                                                ?.color || "#ffffff",
                                          fontSize:
                                             file.metadata.textAnnotation.style
                                                ?.fontSize || "24px",
