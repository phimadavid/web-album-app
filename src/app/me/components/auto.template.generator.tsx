...
import {
   AutoTemplateGeneratorProps,
   TemplateData,
} from "@/app/design/types/template";
import {
   analyzeEventTags,
   getTemplateConfigForCategory,
   isLikelyConcert,
   parseEventTags,
} from "@/app/design/components/event.prediction";
import DesignTemplatesLibrary, {
   DesignTemplate,
} from "@/app/design/components/design.templates.library";

const AutoTemplateGenerator: React.FC<AutoTemplateGeneratorProps> = ({
   albumId,
   Images,
}) => {
   ...
   const [overlayImageIndex, setOverlayImageIndex] = useState<{
   ...
   const [aiOverlayImages, setAiOverlayImages] = useState<{
      [templateId: string]: string[];
   }>({});
   const [aiOverlayPrompts, setAiOverlayPrompts] = useState<{
      [templateId: string]: string[];
   }>({});
   const [isGeneratingOverlay, setIsGeneratingOverlay] = useState<{
   ...
   const generateAIOverlayImage = async (
      templateId: string,
      prompt: string
   ) => {
      ...
      try {
         ...
         if (response && response.length > 0) {
            ...
            if (
               !(templateId in aiOverlayImages) ||
               aiOverlayImages[templateId].length === 0
            ) {
   ...
   const cycleOverlayImageUpdated = (
      templateId: string,
      direction: "next" | "prev"
   ) => {
   ...
   const autoGenerateAIOverlays = async (templateId: string) => {
      ...
      const randomPrompt =
         defaultOverlayPrompts[
            Math.floor(Math.random() * defaultOverlayPrompts.length)
         ];
   ...
   useEffect(() => {
      if (Images && Images.length > 0) {
         ...
         if (concertDetected) {
            const concertCategory = analyzeEventTags([
               "concert",
               "stage",
               "crowd",
            ]);
            ...
            const templateConfig =
               getTemplateConfigForCategory(eventCategoryObj);
      ...
      try {
         ...
         const templateToApply = {
            ...
            template: {
               ...
               previewUrl:
                  selectedTemplateData.compositeImage ||
         ...
         setError(
            err instanceof Error
               ? err
               : new Error("Failed to navigate to album book")
         );
   ...
   const generateSingleTemplate = async (
      ...
      try {
         ...
         setTemplates(prevTemplates =>
            prevTemplates.map(t =>
               t.id === templateId ? { ...t, imageLoading: true } : t
            )
         );
      ...
      try {
         ...
         for (let i = 0; i < numberOfTemplates; i++) {
            ...
            const styleIndex = Math.floor(
               Math.random() * templateStyles.length
            );
         ...
         setError(
            err instanceof Error
               ? err
               : new Error("Failed to generate templates")
         );
   ...
   const handleGenerateNewTemplate = async () => {
      ...
      try {
         ...
         if (template) {
            ...
            setTemplates(prev =>
               prev.map(t => (t.id === newTemplateId ? template : t))
            );
   ...
   const handleGenerateCustomTemplate = async () => {
      ...
      try {
         ...
         setError(
            err instanceof Error
               ? err
               : new Error("Failed to generate custom template")
         );
      } finally {
         setIsLoading(false);
      }
      ...
      <div>
         <div className="w-full gap-4">
            <div className="mb-6 p-4 h-auto row-start-1 col-span-4">
               <h3 className="text-xl font-medium mb-2 py-2 text-end">
                  Generate Prompt Custom Book Cover Design
               </h3>
               <div className="flex gap-2 mb-4">
                  ...
                  <button
                     ...
                     disabled={
                        isLoading || Images.length === 0 || isTranslating
                     }
                  >
               ...
               <div className="border-t pt-4 mt-4">
                  <div className="text-end mb-2">
                     ...
                     <p className="text-xs text-gray-500">
                        Automatically generating people, travel, and bonding
                        overlays for your templates
                     </p>
                  </div>
               </div>
         ...
         <div className="grid grid-cols-10 gap-4 p-2 mt-5">
            <div className="col-start-1 col-span-8 p-4">
               <h2 className="text-xl text-end font-bold mb-4">
                  Generated Album Book Cover Designs
               </h2>
               {detectedEventTags.length > 0 && (
                  <div className="p-3 mb-4 bg-blue-50 border-l-4 border-blue-500 text-blue-700">
                     ...
                     <p className="text-sm mt-1">
                        Generate book cover design with {promptWord} themes
                        ...
                        <div className="flex justify-center my-8">
                           <div className="flex flex-col items-center gap-2">
                              ...
                              <p className="text-gray-600">
                                 Generating book design templates...
                              </p>
                           </div>
                        </div>
                     )}
                     ...
                     {templates.length === 0 &&
                        ...
                        Images.length === 0 && (
                           <div className="text-center my-8 p-4 border border-gray-200 rounded-lg bg-gray-50">
                              <p className="text-gray-600">
                                 Please select cover images to generate
                                 templates
                              </p>
                           </div>
                        )}
                     ...
                     {isLoading && templates.length === 0 && (
                        <div className="flex flex-col items-center justify-center p-8">
                           ...
                           <p className="mt-4 text-gray-600">
                              Generating Cover Design based on your images
                              analysis...
                           </p>
                        </div>
                     )}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                           ...
                           {templates.map(template => (
                              <div
                                 ...
                                 onClick={() =>
                                    handleNavigateToAlbumBook(template.id)
                                 }
                              >
                                 <div className="relative h-64">
                                    ...
                                    template.imageLoading ? (
                                       ...
                                       <div className="absolute w-full inset-0 borders">
                                          ...
                                          {(() => {
                                             const settings =
                                                getOverlaySettings(template.id);
                                             ...
                                             return (
                                                <div
                                                   className={`absolute inset-0 z-20 flex justify-center ${positionClass}`}
                                                >
                                                   <div
