...
import {
   identifyEvents,
   cleanupObjectURLs,
} from "@/backend/helpers/exif.helpers";
...
export default function UploadPage({ params }: { params: { id: string } }) {
   ...
   const [suggestedOrganization, setSuggestedOrganization] =
   ...
   useEffect(() => {
      ...
      const withExactDate = organizedImages.filter(
      ...
      const withLocation = organizedImages.filter(img => {
         ...
         if (
            img.metadata?.captureDate &&
            getTimeZoneRegion(img.metadata.captureDate)
         ) {
            return true;
         }
      ...
      const withAiTags = organizedImages.filter(
   ...
   */
   ...
   const processFiles = async (files: File[]) => {
      ...
      try {
         ...
         const enhancedFilesPromises = files.map(async (file, index) => {
            try {
               ...
               if (hfenhancedFile.metadata?.captureDate instanceof Date) {
                  captureLocation = inferLocationFromTimeZone(
                     hfenhancedFile.metadata.captureDate
                  );
               }
         ...
         const enhancedFilesWithNulls = await Promise.all(
            enhancedFilesPromises
         );
         const enhancedFiles = enhancedFilesWithNulls as EnhancedFile[];

         if (enhancedFiles.length === 0 && files.length > 0) {
            setError(
               "Unable to process any of the selected images. Please try again."
            );
         ...
         const batchAnalysisPromises = enhancedFiles.map(
            async (file, index) => {
               const analysisProgress =
                  33 + Math.floor((index / enhancedFiles.length) * 33);
         ...
         if (validMethod === "content") {
            ...
            organizedFiles = organizeImagesWithAI(
               newUploadedImages,
               validMethod as "date" | "location" | "event"
            );
         }
      ...
      (method: string) => {
         ...
         try {
            ...
            if (method === "content") {
               ...
               const sortedGroups = Object.entries(contentGroups).sort(
                  ([groupA], [groupB]) => {
                     if (groupA.startsWith("Event:")) return -1;
                     if (groupB.startsWith("Event:")) return 1;
                     if (groupA === "Portrait" || groupA === "Group Photo")
                        return -1;
                     if (groupB === "Portrait" || groupB === "Group Photo")
                        return 1;
            ...
            for (let i = 0; i < uploadedImages.length; i++) {
               setProcessingProgress(
                  Math.floor((i / uploadedImages.length) * 100)
               );
            }
   ...
   const handleImageUpload = useCallback(
      async (e: React.ChangeEvent<HTMLInputElement>) => {
         ...
         if (invalidFiles.length > 0) {
            setError(
               `Please upload only JPG or PNG files. Invalid files: ${invalidFiles.map(f => f.name).join(", ")}`
            );
            return;
         }
      ...
      (e: React.DragEvent) => {
         ...
         if (e.dataTransfer.files && e.dataTransfer.files.length > 0) {
            ...
            if (files.length !== imageFiles.length) {
               setError(
                  `${files.length - imageFiles.length} non-image files were ignored`
               );
            }
   ...
   useEffect(() => {
      ...
      if (
         ...
         console.log(
            "Album ID changed from",
            prevAlbumIdRef.current,
            "to",
            params.id,
            "- resetting state"
         );
                  ...
                  {!uploadComplete ? (
                     <div>
                           ...
                           <div className="flex flex-col justify-center mb-4 bg-blue-50 p-5 rounded-lg shadow-sm">
                                 ...
                                 <div className="flex items-center">
                                    ...
                                    <span className="text-blue-700 font-medium">
                                       ...
                                       {processingPhase === "analyzing" &&
                                          "Analyzing content using A.I..."}
                                       {processingPhase === "organizing" &&
                                          "Organizing photos using A.I..."}
                                       {processingPhase === "complete" &&
                                          "Processing complete!"}
                                    </span>
                                 </div>
                              ...
                              <div className="mt-3 flex items-center justify-center">
                                 ...
                                 <p className="text-sm text-blue-700">
                                    {processingPhase === "extracting" &&
                                       "AI is analyzing your photos to identify people, objects, and scenes. This helps organize them into meaningful groups."}
                                    {processingPhase === "analyzing" &&
                                       "Converting technical data into useful information about locations, events, and photo content."}
                                    {processingPhase === "organizing" &&
                                       "Creating the perfect arrangement of your photos based on time, location, and content."}
                                    {processingPhase === "complete" &&
                                       "Your photos have been successfully processed and are ready for the next step!"}
                                 </p>
                              </div>
                           </div>
                        )}
                        ...
                        {organizedImages.length > 0 && (
                           <div className="mb-4">
                              <div className="flex flex-col mb-4 flex-wrap gap-2"></div>
                              <div>
                                    ...
                                    <div className="w-full bg-gray-50 rounded-lg p-4 mb-4">
                                       ...
                                       <div className="flex flex-wrap justify-end gap-2">
                                          ...
                                          >
                                             <Check
                                                size={16}
                                                className={`mr-1 ${organizationMethod === "auto" ? "text-white" : "text-blue-600"}`}
                                             />
                                             ...
                                             onClick={() => {
                                                if (
                                                   ...
                                                   const sortedImages = [
                                                      ...
                                                      const dateA = a.metadata
                                                         ?.captureDate
                                                         ? new Date(
                                                              a.metadata.captureDate
                                                           ).getTime()
                                                         : 0;
                                                      const dateB = b.metadata
                                                         ?.captureDate
                                                         ? new Date(
                                                              b.metadata.captureDate
                                                           ).getTime()
                                          ...
                                          >
                                             <Calendar
                                                size={16}
                                                className={`mr-1 ${organizationMethod === "date" ? "text-white" : "text-blue-600"}`}
                                             />
                                             By Date
                                          </button>
                                          ...
                                          >
                                             <MapPin
                                                size={16}
                                                className={`mr-1 ${organizationMethod === "location" ? "text-white" : "text-blue-600"}`}
                                             />
                                             By Location
                                          </button>
                                          ...
                                          >
                                             <Users
                                                size={16}
                                                className={`mr-1 ${organizationMethod === "event" ? "text-white" : "text-blue-600"}`}
                                             />
                                             By Event
                                          </button>
                                       </div>
                                       ...
                                       <div className="mt-3 text-xs text-gray-500 flex items-center justify-end">
                                          {organizationMethod === "auto" && (
                                             <p>
                                                ...
                                                using AI to determine the best
                                                method based on your photos'
                                                metadata.
                                             </p>
                                          )}
                                          {organizationMethod === "date" && (
                                          ...
                                          {organizationMethod ===
                                             "location" && (
                                             <p>
                                                Groups photos by where they were
                                                taken, using GPS data or
                                                location information.
                                             </p>
                                          )}
                                          {organizationMethod === "event" && (
                                             <p>
                                                Organizes photos by detected
                                                events, combining date and
                                 ...
                                 <div className="mb-4 flex justify-end items-start bg-blue-50 rounded-lg text-sm">
                                    <div className="flex p-2">
                                       <div className="flex flex-col text-blue-800">
                                          ...
                                          <p className="mt-1">
                                             Your photos have been automatically
                                             organized using A.I for the best
                                             viewing experience.
                                          </p>
                                       </div>
                                    </div>
