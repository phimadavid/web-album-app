      ...
      const categoriesToGenerate =
         categoryId === "all"
            ? [
                 ...
                 "sports",
                 "corporate",
              ] // Generate from 6 categories for "all"
            : [categoryId];
      ...
      try {
         ...
         if (categoryId === "all") {
            ...
            for (const category of categoriesToGenerate) {
               for (let i = 0; i < 2; i++) {
                  const styleIndex =
                     (categoriesToGenerate.indexOf(category) * 2 + i) %
                     styles.length;
                  const style = styles[styleIndex];
                  const colorPalette =
                     colorPalettes[styleIndex % colorPalettes.length];
         ...
         {isLoading && templates.length === 0 ? (
            ...
            <div className="text-center py-10">
               <p className="text-gray-500">
                  No book cover design found. Select a category to get started.
               </p>
            </div>
         ) : (
            <div className="grid grid-cols-2 gap-4 mt-2 overflow-x-auto pr-2">
               {filteredTemplates.map(template => (
                  <div
                     ...
                     onClick={() =>
                        !template.isLoading && onSelectTemplate(template)
                     }
                  >
