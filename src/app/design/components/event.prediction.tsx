 ...
 */
export const parseEventTags = (
   eventTagsData: string | string[] | undefined
): string[] => {
...
export const analyzeEventTags = (tags: string[]): EventCategory => {
   if (!tags || tags.length === 0) {
      ...
      return (
         eventCategories.find(cat => cat.name === "General Event") ||
         eventCategories[eventCategories.length - 1]
      );
   }
...
export const isLikelyConcert = (tags: string[]): boolean => {
   ...
   const matchCount = tags.filter(tag =>
      concertIndicators.some(
         indicator => tag.includes(indicator) || indicator.includes(tag)
      )
   ).length;
