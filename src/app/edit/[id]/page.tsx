...
const BookAlbumPage = ({ params }: BookAlbumPageProps) => {
      ...
      if (setAlbumData && setIsToLoading) {
         ...
         axios
            ...
            .catch(error => {
               ...
               setErrorMessage(
                  "Failed to refresh album data after adding photos"
               );
            })
            .finally(() => {
   ...
   const handleChangeLayout = async (layout: string): Promise<void> => {
      // Store the current layout for potential revert (declare outside try so catch can access it)
      const previousLayout = albumData?.layoutPage;
      ...
      try {
         ...
         // Clear any individual page layout overrides when setting global layout
         // This ensures both left and right sides follow the same layout consistently
         setPageLayouts([]);

         // Update album data immediately to ensure UI synchronization
         setAlbumData(
         ...
         // Force a brief delay to ensure state is updated before API call
         ...
         if (data.status === 200) {
            toast.success(`Layout changed to ${layout} successfully`, {
               position: "bottom-right",
               autoClose: 3000,
         ...
         // Revert to the actual previous layout on error
         setAlbumData(
            (prevData: AlbumDataProps | null): AlbumDataProps | null =>
               prevData
                  ? {
                       ...prevData,
                       layoutPage: previousLayout || "single",
                    }
                  : null
         );

         toast.error("Failed to change layout. Please try again.", {
            position: "bottom-right",
            autoClose: 3000,
         });
      } finally {
         // Add a longer delay before clearing layout changing state to ensure proper re-render
         setTimeout(() => {
               ...
               <motion.div>
                  {isLayoutChanging || isRefreshing || isToLoading ? (
                     ...
                     <PageSlider
                        ...
                        key={`${albumData?.layoutPage || "default"}-${pageLayouts.length}`}
                     />
                  ) : (
                     <FlippingBook
                        coverColor="#8B4513"
...
const FlippingBook: React.FC<FlippingBookProps> = ({
   ...
   const getLayoutForPage = (pageIndex: number): string => {
      ...
      if (pageLayouts && pageLayouts.length > 0 && pageLayouts[pageIndex]) {
...
const AsideNavigation: React.FC<AsideNavigationProps> = ({
   ...
   // Sync selectedLayoutId with actual album layout
   useEffect(() => {
      if (albumData?.layoutPage) {
         setSelectedLayoutId(albumData.layoutPage);
      }
   }, [albumData?.layoutPage]);
   ...
   const handleLayoutSelect = (layoutId: string): void => {
      ...
      // Call the layout change handler and ensure it's applied consistently
