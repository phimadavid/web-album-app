...
const BookAlbumPage = ({ params }: BookAlbumPageProps) => {
  ...
  const generateAllPagesBackground = async (promptWord: string) => {
    ...
    try {
      ...
      toast.success('Backgrounds generated for all pages (front and back)!', {
        position: 'bottom-right',
        autoClose: 3000,
      });
    } catch (error) {
        ...
        {!isBookOpen ? (
          <motion.div className="flex flex-col gap-5 justify-center items-center bg-white text-black py-3 px-6">
            <div className="flex flex-col justify-center items-center gap-2">
              ...
              <p className="text-sm">
                Click the{' '}
                <span className="italic underline">accept invitation</span>{' '}
...
> = ({
  ...
  setActivePanel,
    ...
    const currentPageCaptions = generatedCaptions.filter(caption => {
      const frontPhotoIndex = (currentPage - 2) * 2;
      return caption.imageIndex >= frontPhotoIndex && caption.imageIndex < frontPhotoIndex + 2;
    });
          ...
          return (
            <>
              <div className="space-y-4 mb-2">
                ...
                <div>
                  <h4 className="font-medium text-sm text-gray-700 mb-2">Album Name</h4>
                  ...
                  <p className="text-xs text-gray-600 mb-4">
                    Edit the album name and click Save, or Cancel to revert changes.
                  </p>
                </div>
                ...
                <div>
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <button
                        onClick={() => generateSingleBackground(backgroundPrompt, startPromptIndex)}
                        disabled={!backgroundPrompt.trim() || isGeneratingBackground}
                        className={`p-3 flex items-center justify-center rounded-full text-sm transition-colors ${!backgroundPrompt.trim() || isGeneratingBackground
                          ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                      ...
                      <button
                        onClick={() => generateAllPagesBackground(backgroundPrompt)}
                        disabled={!backgroundPrompt.trim() || isGeneratingBackground}
                        className={`p-3 flex items-center justify-center rounded-full text-sm transition-colors ${!backgroundPrompt.trim() || isGeneratingBackground
                          ? 'bg-gray-300 text-gray-500 cursor-not-allowed'
                    ...
                    {isContentPage && (
                      <div className="p-3 bg-orange-50 border border-orange-200 rounded">
                        <p className="text-xs text-orange-700 font-medium">
                          Current Paper: {startPromptIndex + 1} (Front & Back)
                        </p>
                        <p className="text-xs text-orange-600 mt-1">
                          Generate backgrounds for both sides of current paper
                        </p>
                      </div>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 gap-3">
                  {layoutLibrary.map((layout) => (
                    <button
                      ...
                      className={`w-full p-3 border-2 rounded-lg transition-all hover:shadow-md ${selectedLayoutId === layout.id
                    ...
                    >
                      <div className="flex items-center space-x-3">
                        ...
                        <div className="flex-1 text-left">
                          <h5 className={`text-sm font-medium ${selectedLayoutId === layout.id ? 'text-purple-700' : 'text-gray-700'
                            }`}>
                            {layout.name}
                          </h5>
                          <p className="text-xs text-gray-500 mt-1">
