...
const PageSlider: React.FC<PageSliderProps> = ({
   ...
   const [showLayoutSelector, setShowLayoutSelector] = useState<{
      pageIndex: number;
   } | null>(null);
   ...
   const getLayoutForPage = (slideIndex: number, pageIndex: number) => {
      if (pageLayouts && pageLayouts.length > 0) {
         ...
         return (
            pageLayouts[actualPageIndex] || albumData?.layoutPage || "single"
         );
      }
      return albumData?.layoutPage || "single";
   };
   ...
   const renderSingleImage = (startIndex: number) => {
      ...
      return (
         <div
            ...
            style={getContainerStyle(image)}
            className={`aspect-square w-full max-w-sm overflow-hidden relative cursor-pointer transition-all duration-200 rounded-lg ${
               isSelected
                  ? "border-4 border-orange-500 shadow-lg scale-105"
                  : "border-2 border-white hover:border-orange-300 hover:shadow-md"
            }`}
               ...
               parsedTextAnnotation.textContent && (
                  <div
                     ...
                     style={{
                        ...
                        fontSize:
                           parsedTextAnnotation?.style?.fontSize || "20px",
                        fontFamily:
                           parsedTextAnnotation?.style?.fontFamily ||
                           "Arial, sans-serif",
                        fontWeight:
                           parsedTextAnnotation?.style?.fontWeight || "normal",
   ...
   const renderPageContent = (
      ...
      const pageBackground =
         pageBackgrounds[backgroundIndex] || albumData?.bookDesign;
         ...
         return (
            <div className="w-full aspect-square flex flex-col justify-center items-center p-6 shadow-lg bg-gray-100 rounded-lg relative">
               ...
               <div className="text-center">
                  ...
                  <p className="text-sm text-gray-600">
                     {isLeftPage ? "Left" : "Right"} page has no images
                  </p>
               </div>
            </div>
         );
                  ...
                  >
                     ...
                     <div className="grid grid-cols-2 gap-2">
                        {availableLayouts.map(layoutOption => {
                           const currentPageLayout = getLayoutForPage(
                              currentSlide,
                              showLayoutSelector.pageIndex
                           );
                           return (
                              <button
                                 key={layoutOption}
                                 ...
                                 className={`p-2 text-xs rounded-md transition-all hover:scale-105 ${
