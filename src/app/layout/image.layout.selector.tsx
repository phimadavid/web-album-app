...
const PhotoAlbumLayoutSelector: React.FC<PhotoAlbumLayoutSelectorProps> = ({
   ...
   const [selectedLayout, setSelectedLayout] = useState<LayoutType>(
      (defaultLayout as LayoutType) || "random"
   );
   ...
   const imagesToUse =
      sampleImages.length > 0 ? sampleImages : defaultSampleImages;
   ...
   const renderLayoutPreview = (layout: LayoutType) => {
      ...
      const selectedClasses =
         selectedLayout === layout ? "ring-2 ring-green-500" : "";
         ...
         <motion.div className={`${baseClasses} ${selectedClasses}`}>
            {selectedLayout === layout && (
               <motion.div
                  ...
                  initial={{ scale: 0 }}
                  animate={{
                     scale: animateSelection ? [1, 1.2, 1] : 1,
                  }}
                  ...
                  {[0, 1, 2, 3].map(index => (
                     ...
                     >
                        <Image
                           src={
                              imagesToUse[
                                 (imageOffset + index) % imagesToUse.length
                              ]
                           }
                           alt=""
                           fill
            ...
            return (
               <div className="absolute inset-0 m-8">
                  ...
                  <div className="absolute top-0 right-0 w-2/5 h-2/5 bg-white shadow-md overflow-hidden">
                     <Image
                        src={
                           imagesToUse[(imageOffset + 1) % imagesToUse.length]
                        }
                        alt=""
                        fill
                        style={{ objectFit: "cover" }}
                  ...
                  <div className="absolute bottom-0 right-0 w-2/5 h-2/5 bg-white shadow-md overflow-hidden">
                     <Image
                        src={
                           imagesToUse[(imageOffset + 2) % imagesToUse.length]
                        }
                        alt=""
                        fill
                        style={{ objectFit: "cover" }}
                  ...
                  <div className="absolute top-8 right-4 w-16 h-20 bg-white shadow-lg transform -rotate-6 p-1">
                     <div className="w-full h-3/4 bg-gray-200 overflow-hidden relative">
                        <Image
                           src={
                              imagesToUse[
                                 (imageOffset + 1) % imagesToUse.length
                              ]
                           }
                           alt=""
                           fill
                  ...
                  <div className="absolute bottom-4 left-6 w-16 h-20 bg-white shadow-lg transform rotate-3 p-1">
                     <div className="w-full h-3/4 bg-gray-200 overflow-hidden relative">
                        <Image
                           src={
                              imagesToUse[
                                 (imageOffset + 2) % imagesToUse.length
                              ]
                           }
                           alt=""
                           fill
            ...
            return (
               <div className="absolute inset-0 m-8">
                  ...
                  <div className="absolute bottom-2 right-6 w-8 h-8 bg-white shadow-md overflow-hidden transform -rotate-45 rounded-full">
                     <Image
                        src={
                           imagesToUse[(imageOffset + 3) % imagesToUse.length]
                        }
                        alt=""
                        fill
                        style={{ objectFit: "cover" }}
                  ...
                  <div className="absolute top-16 left-12 w-6 h-8 bg-white shadow-md overflow-hidden transform rotate-45">
                     <Image
                        src={
                           imagesToUse[(imageOffset + 4) % imagesToUse.length]
                        }
                        alt=""
                        fill
                        style={{ objectFit: "cover" }}
         ...
         <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {layoutOptions.map((option, index) => (
               <motion.div
                  ...
                  onKeyDown={e =>
                     e.key === "Enter" && handleLayoutSelect(option.id)
                  }
                  className="group"
               >
                  ...
                  <div className="p-4 text-center">
                        ...
                        <div className="relative inline-block ml-1">
                           <button
                              ...
                              onClick={e => {
                                 e.stopPropagation();
                                 setShowTooltip(
                                    showTooltip === option.id ? null : option.id
                                 );
                              }}
                     ...
                     {option.maxImages && (
                        <p className="text-sm text-gray-600">
                           Up to {option.maxImages} photos per book
                        </p>
                     )}
                     {showNotifications && selectedLayout === option.id && (
                        <motion.p
                           ...
                           transition={{ duration: 0.3 }}
                        >
                           Your book will be approximately{" "}
                           {calculatePages(option.id)} pages with this layout
                        </motion.p>
                     )}
                  </div>
               </motion.div>
