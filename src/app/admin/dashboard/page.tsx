...
import { Package, DollarSign, CheckCircle, XCircle, Clock, Truck, Palette, BookOpen } from "lucide-react";
...
const AdminDashboard = () => {
    ...
    const calculateStats = (orderData: Order[]) => {
        const stats = orderData.reduce((acc, order) => {
            ...
            const hasAiArt = items.some(item =>
                item.albumName?.toLowerCase().includes('ai') ||
                item.albumName?.toLowerCase().includes('generated')
            );
            const hasAlbum = items.some(item =>
                item.bookFormat || item.coverType
            );
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    ...
                    <p className="text-gray-600">
                        Track and manage all user orders for AI-generated art and album books
                    </p>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6 mb-8">
                    ...
                    <Card className="p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-gray-600">AI Art Orders</p>
                                <p className="text-2xl font-bold text-gray-900">{stats.aiArtOrders}</p>
                            </div>
                        </div>
                    </Card>
                    <Card className="p-6">
                        <div className="flex items-center">
                            ...
                            <div className="ml-4">
                                <p className="text-sm font-medium text-gray-600">Album Orders</p>
                                <p className="text-2xl font-bold text-gray-900">{stats.albumOrders}</p>
                            </div>
                        </div>
                    </Card>
                </div>
                                ...
                                <Card key={order.id} className="p-6 hover:shadow-lg transition-shadow">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-900 font-semibold">${parseFloat(order.total.toString()).toFixed(2)}</p>
                                        </div>
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-900">{Array.isArray(order.items) ? order.items.length : 0} item(s)</p>
                                        </div>
                                    </div>
                                        ...
                                        <div className="space-y-2">
                                            {Array.isArray(order.items) ? order.items.map((item, index) => (
                                                <div key={index} className="flex justify-between items-center text-sm">
                                                    <div>
                                                        <span className="font-medium">{item.albumName || 'Album'}</span>
                                                        {item.bookFormat && <span className="text-gray-600 ml-2">({item.bookFormat})</span>}
                                                        <span className="text-gray-600 ml-2">× {item.quantity}</span>
                                                    </div>
                                                    <span className="font-medium">${parseFloat(item.totalPrice.toString()).toFixed(2)}</span>
                                                </div>
                                            )) : (
